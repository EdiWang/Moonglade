...
function Create-AppServicePlan($aspName, $rsgName, $regionName, $useLinuxPlanWithDocker) {
    ...
    if (!$planExists) {
        if ($useLinuxPlanWithDocker) {
            az appservice plan create -n $aspName -g $rsgName --is-linux --sku P0V3 --location $regionName
        } else {
            az appservice plan create -n $aspName -g $rsgName --sku P0V3 --location $regionName
        }
    }
}
...
function Create-StorageAccount($storageAccountName, $rsgName, $regionName) {
    ...
    if (!$storageAccountExists) {
        ...
        az storage account create --name $storageAccountName --resource-group $rsgName --location $regionName --sku Standard_LRS --kind StorageV2 --allow-blob-public-access true
    }
}
...
function Create-WebApp($webAppName, $rsgName, $aspName, $useLinuxPlanWithDocker, $dockerImageName) {
    ...
    if (!$appExists) {
        ...
        if ($useLinuxPlanWithDocker) {
            ...
            az webapp create -g $rsgName -p $aspName -n $webAppName --container-image-name $dockerImageName
        } else {
            Write-Host "Using Windows Plan with deployment from GitHub"
            az webapp create -g $rsgName -p $aspName -n $webAppName --runtime "DOTNET|8.0"
        }
...
function Update-WebAppConfig($webAppName, $rsgName, $sqlConnStr, $storageConn, $storageContainerName, $useLinuxPlanWithDocker) {
    ...
    if ($useLinuxPlanWithDocker) {
        ...
        az webapp config appsettings set -g $rsgName -n $webAppName --settings ForwardedHeaders__UseForwardedHeaders=true
        az webapp config appsettings set -g $rsgName -n $webAppName --settings ASPNETCORE_FORWARDEDHEADERS_ENABLED=true
    } else {
