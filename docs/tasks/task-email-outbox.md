...
Before this migration, Moonglade sent email notifications from the web application through `Moonglade.Email.Client`, which posted to an Azure Function `/api/enqueue` endpoint. The Function validated the payload, stored it in Azure Storage Queue, and processed it with a queue trigger that built template-based messages and sent them through SMTP or Azure Communication Services.
...
- Add database outbox persistence, a background worker, web integration, configuration updates, and old Function client cleanup.
...
| 8 | Update README, AGENTS.md, and related docs | Tasks 5-7 | `git diff --check`, build | Completed |
...
Long-lived documentation now describes the database-backed email outbox, supported email providers, at-least-once delivery semantics, worker configuration, and module ownership. Obsolete Function API UI resource keys were removed.
...
| 2026-08-02 | `dotnet build src\Moonglade.Web\Moonglade.Web.csproj` | Passed | Build succeeded with 0 warnings and 0 errors after final documentation and localization cleanup. |
| 2026-08-02 | `dotnet build src\Moonglade.slnx --no-restore` | Passed | Solution build succeeded with 0 warnings and 0 errors after final documentation and localization cleanup. |
...
- Final documentation cleanup removed unused Function API localization keys and documented that rare duplicate notifications are possible because delivery is at-least-once.
