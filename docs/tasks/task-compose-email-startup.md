...
No.1-5 are complete. Email options use non-fatal validation and a shared capability status; unavailable workers and notification producers perform no outbox work. The test-email API now rejects `NotConfigured` and `Invalid` with HTTP 503 ProblemDetails, rejects capability or blog-setting disablement with HTTP 409 ProblemDetails, and returns HTTP 500 ProblemDetails if event publication fails. It publishes `TestEmailEvent` and returns `true` only when both capability and blog email sending are enabled. The Notification page now consumes the shared status and bound options instead of raw configuration readiness checks. It renders localized setup guidance for `NotConfigured`, safe validation details for `Invalid`, a distinct worker-disabled notice for `Disabled`, and disables the test-email button whenever capability or blog settings make sending unavailable. No.6 remains not started.
