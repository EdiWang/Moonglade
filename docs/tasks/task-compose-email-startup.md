...
No.1-3 are complete. Email options are bound without fatal options-pipeline validators, the singleton capability status reuses the existing rules, and startup emits one state-appropriate log. `EmailOutboxWorker` now exits before reading execution intervals, creating a scope, or resolving the outbox processor whenever the capability is unavailable. Comment, comment-reply, Webmention, and test-email event handlers also return before serialization or enqueueing. Because unavailable execution never resolves the processor, existing pending outbox rows are left unchanged. Tests cover all unavailable worker states, the available polling path, all four notification producers, and their existing available behavior. No.4-6 remain not started.
