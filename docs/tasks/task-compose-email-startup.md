...
No.1-6 are complete. Email is now an optional application capability with one non-fatal status shared by startup logging, the outbox worker, notification producers, the test-email API, and the Admin Portal. README deployment guidance describes missing, invalid, and deliberately disabled states, retained pending messages, restart expectations, and secret handling. A clean isolated Compose build with zero `Email__*` overrides started successfully from the repository defaults: the Web container remained running with zero restarts, `/health` and `/health/ready` returned HTTP 200, and startup emitted exactly one `NotConfigured` warning with no email validation exception or configuration error. An authenticated request to the live Notification page rendered the `NotConfigured` setup guidance, omitted the invalid-configuration message, and disabled the test-email button. The isolated containers, network, database volume, and image volumes were removed after verification; no email was sent.
...
| 2026-08-23 | `dotnet run --project src/Tests/Moonglade.Email.Tests/Moonglade.Email.Tests.csproj --no-restore` after No.2 | Passed | xUnit v3 executed 77 tests; 77 passed, 0 failed |
| 2026-08-23 | `dotnet build src/Moonglade.Web/Moonglade.Web.csproj --no-restore` after No.2 | Passed | Web host and all referenced projects built with 0 warnings and 0 errors |
| 2026-08-23 | `dotnet run --project src/Tests/Moonglade.Email.Tests/Moonglade.Email.Tests.csproj --no-restore` after No.3 | Passed | xUnit v3 executed 85 tests; 85 passed, 0 failed |
| 2026-08-23 | `dotnet build src/Moonglade.Web/Moonglade.Web.csproj --no-restore` after No.3 | Passed | Web host and all referenced projects built with 0 warnings and 0 errors |
| 2026-08-23 | `node --check src/Moonglade.Web/wwwroot/js/app/admin.settings.notification.mjs` after No.4 | Passed | The updated test-email client script passed Node.js syntax validation |
| 2026-08-23 | `dotnet run --project src/Tests/Moonglade.Web.Tests/Moonglade.Web.Tests.csproj --no-restore` after No.4 | Passed | xUnit v3 executed 181 tests; 181 passed, 0 failed |
| 2026-08-23 | `dotnet build src/Moonglade.Web/Moonglade.Web.csproj --no-restore` after No.4 | Passed | Web host and all referenced projects built with 0 warnings and 0 errors |
| 2026-08-23 | `node --check src/Moonglade.Web/wwwroot/js/app/admin.settings.notification.mjs` after No.5 | Passed | The unavailable-button guard and updated test-email script passed Node.js syntax validation |
| 2026-08-23 | `dotnet run --project src/Tests/Moonglade.Web.Tests/Moonglade.Web.Tests.csproj --no-restore` after No.5 | Passed | xUnit v3 executed 190 tests, including all four page capability states and four localization resources; 190 passed, 0 failed |
| 2026-08-23 | `dotnet build src/Moonglade.Web/Moonglade.Web.csproj --no-restore` after No.5 | Passed | Web host, Razor pages, and all referenced projects built with 0 warnings and 0 errors |
| 2026-08-23 | `docker compose config --quiet` and parsed Compose Web environment before No.6 smoke test | Passed | Compose configuration was valid and contained zero `Email__*` environment overrides |
| 2026-08-23 | `dotnet run --project src/Tests/Moonglade.Email.Tests/Moonglade.Email.Tests.csproj --no-restore` after No.6 documentation update | Passed | xUnit v3 executed 85 tests; 85 passed, 0 failed |
| 2026-08-23 | `dotnet run --project src/Tests/Moonglade.Web.Tests/Moonglade.Web.Tests.csproj --no-restore` after No.6 documentation update | Passed | xUnit v3 executed 190 tests; 190 passed, 0 failed |
| 2026-08-23 | `dotnet build src/Moonglade.Web/Moonglade.Web.csproj --no-restore` after No.6 documentation update | Passed | Web host, Razor pages, and all referenced projects built with 0 warnings and 0 errors |
| 2026-08-23 | `docker compose --project-name moonglade-codex-email-smoke up --detach --build` with fresh project volumes and no Email overrides | Passed | Current working tree image built with 0 warnings and 0 errors; Web remained running with restart count 0; `/health` and `/health/ready` returned HTTP 200; logs contained one `NotConfigured` warning, no `OptionsValidationException`, and no invalid-configuration error |
| 2026-08-23 | Authenticated HTTP rendering of `/admin/settings/notification` in the isolated Compose deployment | Passed | Page returned HTTP 200 with `NotConfigured` state and localized setup guidance, did not render the invalid-configuration message, and rendered the test-email button disabled; no email was sent |
| 2026-08-23 | `docker compose --project-name moonglade-codex-email-smoke down --volumes --remove-orphans` | Passed | Removed only the isolated smoke containers, network, fresh database volume, and two fresh image volumes after verification |
