...
No.1-4 are complete. Email options use non-fatal validation and a shared capability status; unavailable workers and notification producers perform no outbox work. The test-email API now rejects `NotConfigured` and `Invalid` with HTTP 503 ProblemDetails, rejects capability or blog-setting disablement with HTTP 409 ProblemDetails, and returns HTTP 500 ProblemDetails if event publication fails. It publishes `TestEmailEvent` and returns `true` only when both capability and blog email sending are enabled. The Admin Portal script now shows its queued-success toast only when that success payload is exactly `true`; existing `fetch2` handling displays the ProblemDetails `detail` for non-success responses. No.5-6 remain not started.
