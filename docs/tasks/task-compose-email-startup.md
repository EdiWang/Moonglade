...
No.1 and No.2 are complete. Email options are now bound without fatal options-pipeline validators, while the existing validator rules are explicitly invoked by the singleton capability status. A lightweight hosted reporter resolves that status during startup and emits exactly one state-appropriate log: Warning for `NotConfigured`, Error with secret-safe details for `Invalid`, or Information for `Disabled` and `Available`. Host tests cover missing provider settings, invalid provider settings, invalid worker settings, deliberate disablement, valid configuration, safe logging, and service resolution. No.3-6 remain not started; the worker and notification producers do not yet guard their execution with the shared status.
