...
Task No. 9 and the planned filesystem-only image-storage implementation are complete. Both PR #999 inline review findings were confirmed and fixed. Primary and original writes now reject null image bytes with `ArgumentNullException` and empty image bytes with `ArgumentException` before a file is created. `DeleteAsync` performs its synchronous validation/delete work and returns `Task.CompletedTask` directly. Four new primary/original regression cases pass, bringing `Moonglade.ImageStorage.Tests` to 74 tests; the Web build and all 175 Web tests also pass. The unrelated default Compose email startup failure was retained and subsequently resolved in `docs/tasks/task-compose-email-startup.md`. Live Azure deployment and mount validation remain explicitly deferred to the user; no Azure resource query, validation, what-if, or deployment was performed.
...
- **Default Compose startup was blocked by unrelated email option validation:** `appsettings.json` selects Azure Communication while its required ACS values are empty, and the supplied Compose environment does not override them. Resolution for the image-storage isolation run: supply task-only SMTP values and disable the outbox worker; no Email configuration changed in the image-storage work. The product behavior was subsequently fixed and verified separately in `docs/tasks/task-compose-email-startup.md` so missing Email configuration now leaves the optional capability inactive without blocking startup.
...
- The independent default Compose email configuration prerequisite was resolved under `docs/tasks/task-compose-email-startup.md`; `docker compose up -d` now becomes healthy without Email environment overrides.
