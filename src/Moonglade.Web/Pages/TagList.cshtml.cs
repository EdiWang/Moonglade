...
public class TagListModel : PageModel
{
    ...
    public async Task<IActionResult> OnGet(string normalizedName)
    {
        ...
        var count = await _cache.GetOrCreateAsync(CachePartition.PostCountTag.ToString(), tagResponse.Id.ToString(), _ => _mediator.Send(new CountPostQuery(CountType.Tag, TagId: tagResponse.Id)));
